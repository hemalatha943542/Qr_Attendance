...
# 📷 QR SCANNER — Pure HTML/JS (No extra libraries needed!)
st.markdown('<h2 id="qr-scanner">📷 QR Scanner</h2>', unsafe_allow_html=True)
st.info("📱 Camera-வை QR code-இல் காட்டுங்க — Present mark ஆகும்!")

if 'last_scanned' not in st.session_state:
    st.session_state.last_scanned = ""

scanned_roll = st.query_params.get("scanned", "")
if scanned_roll and scanned_roll != st.session_state.last_scanned:
    st.session_state.last_scanned = scanned_roll
    student_name, status = mark_present_by_scan(scanned_roll)
    if student_name:
        if status == "updated":
            st.success(f"✅ {student_name} — Absent → Present Marked!")
        elif status == "already":
            st.info(f"ℹ️ {student_name} — Already Present!")
        else:
            st.success(f"✅ {student_name} — Present Marked!")
    else:
        st.error(f"❌ Student not found: {scanned_roll}")
...
function startCamera() {
  ...
  .catch(err => {
    setStatus("❌ Camera access கொடுங்க! Allow பண்ணுங்க.", "error");
    btn.disabled  = false;
...
result = components.html(scanner_html, height=520, scrolling=False)

# Manual roll input as fallback
st.markdown("#### அல்லது Manual-ஆ Roll Number போடுங்க:")
col_m1, col_m2 = st.columns([3, 1])
with col_m1:
    manual_roll = st.text_input("Roll Number", key="manual_roll", label_visibility="collapsed", placeholder="Roll number type பண்ணுங்க...")
with col_m2:
    if st.button("✅ Mark Present", use_container_width=True):
        if manual_roll:
            student_name, status_val = mark_present_by_scan(manual_roll)
            if student_name:
                if status_val == "updated":
                    st.success(f"✅ {student_name} — Absent → Present!")
                elif status_val == "already":
                    st.info(f"ℹ️ {student_name} — Already Present!")
                else:
                    st.success(f"✅ {student_name} — Present Marked!")
            else:
                st.error(f"❌ Student not found: {manual_roll}")
        else:
            st.warning("Roll number போடுங்க!")
