...
with col1: name = st.text_input("Student Name")
with col2: roll = st.text_input("Roll Number")
with col3: exam = st.text_input("Exam Number")
...
if students:
    ...
    h0.markdown("**S.No**"); h1.markdown("**Name**"); h2.markdown("**Roll Number**")
    h3.markdown("**Exam Number**"); h4.markdown("**QR Code**"); h5.markdown("**Delete**")
    st.markdown("---")
...
if st.session_state.scan_result_name:
    ...
    else:                st.error(f"❌ Student not found!")

# ✅ streamlit-js-eval: JS லிருந்து Python-க்கு value pass பண்ணும்
scanned_value = streamlit_js_eval(js_expressions="window._scannedQR", key="qr_val")

if scanned_value and scanned_value != st.session_state.last_scanned:
    st.session_state.last_scanned = scanned_value
    name_found, status_val = mark_present_by_roll(scanned_value)
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  #status {
    width:100%; max-width:380px; padding:12px 16px; border-radius:10px;
...
function tick() {
  ...
  if (video.readyState === video.HAVE_ENOUGH_DATA) {
    ...
    if (code && code.data && !cooldown) {
      cooldown = true;
      setStatus("⏳ Marking: " + code.data, "info");

      // ✅ parent window-ல global variable set பண்ணு
      // streamlit_js_eval இதை read பண்ணும்
      window.parent._scannedQR = code.data;

      setTimeout(() => {
        cooldown = false;
        window.parent._scannedQR = "";
      }, 4000);
    }
  }
  requestAnimationFrame(tick);
...
if records:
    ...
    with col_rp:
        ...
        if rep_present:
            ...
            rp0.markdown("**Name**"); rp1.markdown("**Roll No**"); rp2.markdown("**Exam No**")
            st.markdown("---")
            for i, r in enumerate(rep_present, 1):
                rpp0,rpp1,rpp2 = st.columns([2,2,2])
                rpp0.write(f"{i}. {r[0]}"); rpp1.write(r[1]); rpp2.write(r[2] if r[2] else "-")
        else: st.info("No present records!")
    with col_ra:
        st.markdown(f"### ❌ Absent List ({len(rep_absent)})")
        if rep_absent:
            ra0,ra1,ra2 = st.columns([2,2,2])
            ra0.markdown("**Name**"); ra1.markdown("**Roll No**"); ra2.markdown("**Exam No**")
            st.markdown("---")
            for i, r in enumerate(rep_absent, 1):
                raa0,raa1,raa2 = st.columns([2,2,2])
                raa0.write(f"{i}. {r[0]}"); raa1.write(r[1]); raa2.write(r[2] if r[2] else "-")
