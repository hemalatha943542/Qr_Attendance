...
# ✅ Query params மூலம் scanned roll receive பண்ணு
...
# ✅ Scanner HTML — Query Params method (iframe-safe)
current_url = "?"  # Streamlit handles the base URL
...
function tick() {
  ...
  if (video.readyState === video.HAVE_ENOUGH_DATA) {
    ...
    if (code && code.data && !cooldown) {
      ...
      // ✅ Parent window URL-ல் query param சேர்க்கு — Streamlit read பண்ணும்
      const parentUrl = window.parent.location.href.split('?')[0];
      window.parent.location.href = parentUrl + "?roll=" + encodeURIComponent(code.data);
    }
  }
  requestAnimationFrame(tick);
}
